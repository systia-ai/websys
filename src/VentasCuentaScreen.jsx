...
function inyectarCostoReparacionSiFalta(lineas, rep) {
  ...
  return [
    ...lineas,
    {
      ...
      producto_id: 0,
      cantidad: 1,
      descripcion: String(rep.descripcion_equipo ?? 'Costo de reparación').trim() || 'Costo de reparación',
...
function buildLineasDesdeServidor({ movs, reps, pagos, productosPorId = new Map() }) {
  ...
  for (const m of movs) {
    ...
    lineas.push({
      ...
      cantidad: cant,
      descripcion: String(m.descripcion ?? 'Sin descripción').replace(/^\[VENTA\]\s*/i, '') || 'Sin descripción',
  ...
  for (const r of reps) {
    ...
    lineas.push({
      ...
      cantidad: cant,
      descripcion: String(r.descripcion ?? 'Sin descripción').replace(/^\[REPARACI[OÓ]N\]\s*/i, '') || 'Sin descripción',
      precioUnitario: costo,
      subtotal: cant * costo,
    })
  }
